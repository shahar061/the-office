...
// ── UI Mockups (separate collapsible panel) ──

function UIMockupsToolbox() {
  const uiDesigns = useArtifactStore((s) => s.uiDesigns);
  if (uiDesigns.length === 0) return null;
  ...
  return (
    <DraggablePanel id="imagine-ui-mockups" title="UI Mockups" defaultPosition={{ top: 200, right: 12 }}>
      {uiDesigns.map((m) => (
        <div key={m.filename} style={rowStyle(true, uiUxColor)} onClick={() => handleOpen(m.filename)} title={m.caption}>
